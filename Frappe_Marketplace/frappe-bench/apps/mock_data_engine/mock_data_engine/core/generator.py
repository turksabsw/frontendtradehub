...
if TYPE_CHECKING:
    ...
    from mock_data_engine.core.dependency_resolver import (
        DependencyResolver,
        GenerationPlan,
    )
...
class MockDataGenerator:
    ...
    def __init__(
        ...
        # Store initial locale - may be overridden by industry faker hints
        ...
        self._global_context: Optional["GlobalContextManager"] = None
    ...
    def _resolve_configuration(self) -> "ResolvedConfig":
        ...
        Returns:
            ResolvedConfig with all resolved values.
        """
        from mock_data_engine.core.config_resolver import (
            ConfigResolver,
            ResolvedConfig,
        )
        ...
        resolved_config = resolver.resolve_all()
        
        # Apply industry faker hints if available (locale override)
        if profile and hasattr(profile, 'industry') and profile.industry:
            try:
                ind_doc = frappe.get_doc("Mock Industry", profile.industry)
                if hasattr(ind_doc, "get_faker_hints"):
                    faker_hints = ind_doc.get_faker_hints()
                    if faker_hints.get("locale"):
                        # Override locale with industry hint
                        self._locale = faker_hints["locale"]
                        resolved_config.locale = faker_hints["locale"]
            except Exception:
                pass  # Ignore errors, use default locale
    ...
    def _generate_batch(
        ...
        # BATCH AI MODE: Generate multiple records per AI call (5-10x faster)
        if config.use_ai and self._enable_ai_generation and count > 1:
            ai_batch_size = min(5, count)  # 5 records per AI call
            remaining = count
            current_idx = start_idx
            
            while remaining > 0 and not self._aborted:
                batch_count = min(ai_batch_size, remaining)
                
                try:
                    # Generate batch of records in one AI call
                    batch_data_list = self._generate_with_ai_batch(
                        doctype=doctype,
                        meta=meta,
                        config=config,
                    ...
                    if batch_data_list:
                        # Insert all records from batch
                        for record_data in batch_data_list:
                            if self._aborted:
                                break
                            ...
                            if doc_name:
                                ...
                                # Update global context for cross-DocType consistency
                                try:
                                    if doctype == "Organization":
                                        self.global_context.add_organization(doc_name, record_data)
                                    elif doctype == "Tenant":
                                        self.global_context.add_tenant(doc_name, record_data)
                                    elif doctype == "Seller Profile":
                                        self.global_context.add_seller_profile(doc_name, record_data)
                                    elif doctype == "Buyer Profile":
                                        self.global_context.add_buyer_profile(doc_name, record_data)
                                    elif doctype == "Listing":
                                        self.global_context.add_listing(doc_name, record_data)
                                    elif doctype == "Category":
                                        self.global_context.add_category(doc_name, record_data)
                                    elif doctype == "Brand":
                                        self.global_context.add_brand(doc_name, record_data)
                                except Exception:
                                    pass  # Don't fail generation if global context update fails
                        ...
                        remaining -= len(batch_data_list)
                        current_idx += len(batch_data_list)
                    else:
                        # AI batch failed - retry with smaller batch or raise error
                        if batch_count > 1:
                            # Try smaller batch
                            batch_count = max(1, batch_count // 2)
                            continue
                        else:
                            # Even single record failed - raise error
                            raise Exception(
                                f"AI batch generation failed for {doctype}. "
                                "AI generation is required - cannot fallback to Faker. "
                                "Please check API keys and rate limits."
                            )
                        
                except Exception as e:
                    error_msg = str(e)
                    frappe.log_error(
                        message=f"Batch AI generation failed for {doctype}: {error_msg}",
                        title="Mock Batch AI Error"
                    )
                    # Don't fallback to Faker - raise error
                    raise Exception(
                        f"AI generation failed for {doctype}. "
                        "All records must be generated with AI. "
                        "Please check API keys, rate limits, and retry."
                    ) from e
            
            # If batch AI completed all records, return early
            if remaining == 0:
                return

        # INDIVIDUAL GENERATION MODE (Faker or fallback from batch AI)
        for i in range(count):
            ...
            try:
                ...
                if doc_name:
                    ...
                    frappe.db.commit()  # Persist each record immediately
                    self.record_cache.add_record(
                        doctype=doctype,
                        name=doc_name,
                        data=record_data,
    ...
    def _generate_with_ai_batch(self, doctype, meta, config, batch_size=5):
        """
        Generate multiple records in a single AI call (batch mode for efficiency).
        
        This significantly reduces API calls and rate limit issues.
        Example: 100 records = 20 AI calls (5 records per call) instead of 100 calls.
        
        Args:
            doctype: The DocType to generate data for
            meta: DocTypeMeta instance
            config: DocTypeGenerationConfig
            batch_size: Number of records to generate per AI call (default: 5)
        ...
        try:
            ...
            provider_name = (settings.primary_llm_provider or "Gemini").strip()
            temperature = getattr(settings, "ai_temperature", 0.7) or 0.7

            # Read industry context (same as single record generation)
            industry_context = ""
            industry_ai_context = None
            if hasattr(self, "_request_doc") and self._request_doc:
                profile_name = getattr(self._request_doc, "profile", None)
                if profile_name:
                    try:
                        ...
                        if getattr(profile, "industry", None):
                            try:
                                ...
                                industry_context = f"Industry: {ind_doc.industry_name or ind_doc.name}"
                                if getattr(ind_doc, "description", None):
                                    industry_context += f" - {ind_doc.description}"
                                
                                if industry_ai_context:
                                    if industry_ai_context.get("typical_company_types"):
                                        companies = ", ".join(industry_ai_context["typical_company_types"][:10])
                                        industry_context += f"\nTypical Company Types: {companies}"
                                    if industry_ai_context.get("typical_products_services"):
                                        products = ", ".join(industry_ai_context["typical_products_services"][:10])
                                        industry_context += f"\nTypical Products/Services: {products}"
                            except Exception:
                                pass
                        prof_temp = getattr(profile, "ai_temperature", None)
                        if prof_temp and float(prof_temp) > 0:
                            temperature = float(prof_temp)
                    except Exception:
                        pass

            locale = self._locale or "tr_TR"
            locale_lang = "Turkish" if "tr" in locale.lower() else "English"

            # Build field descriptions (same as single record)
            fields_desc = []
            select_options = {}
            link_options = {}
            required_fields = []

            for field_info in meta.get_data_fields():
                fn = field_info.fieldname
                ft = field_info.fieldtype
                if fn == "name":
                    continue
                desc = "- " + fn + " (" + ft + ")"
                if ft == "Select" and field_info.options:
                    opts = [o.strip() for o in field_info.options.split("\n") if o.strip()]
                    select_options[fn] = opts
                    desc += " [ONLY: " + ", ".join(opts) + "]"
                elif ft == "Link" and field_info.options:
                    try:
                        existing = frappe.get_all(field_info.options, limit=20, pluck="name")
                        if existing:
                            link_options[fn] = existing
                            desc += " [use one of: " + ", ".join(existing[:10]) + "]"
                        else:
                            desc += " [no records, use null]"
                    except Exception:
                        pass
                elif ft == "Check":
                    desc += " [0 or 1]"
            ...
            fields_text = "\n".join(fields_desc)
            req_text = ", ".join(required_fields) if required_fields else "none"

            # Build consistency context: related records and previous records
            consistency_context = ""
            
            # Add seed for reproducibility hint
            if self._seed:
                consistency_context += f"\nConsistency Seed: {self._seed} (use this for generating consistent patterns)\n"
            
            # Add GLOBAL CONTEXT for cross-DocType consistency (tr_tradehub app)
            try:
                global_context_str = self.global_context.get_global_context_string(doctype)
                if global_context_str:
                    consistency_context += f"\n=== GLOBAL CONTEXT (Cross-DocType Consistency) ===\n"
                    consistency_context += global_context_str + "\n"
                    consistency_context += "=== END GLOBAL CONTEXT ===\n"
            except Exception:
                pass
            
            # Add context from previously generated records of the same DocType
            try:
                existing_records = self.record_cache.get_records(doctype)
                if existing_records:
                    # Show sample of previous records for consistency
                    sample_size = min(3, len(existing_records))
                    consistency_context += f"\nPreviously generated {len(existing_records)} records of this DocType. "
                    consistency_context += f"Maintain consistency with existing records. Sample fields from previous records:\n"
                    for i, entry in enumerate(existing_records[:sample_size]):
                        # Show key fields for context
                        key_fields = {k: v for k, v in list(entry.data.items())[:5] if v is not None}
                        if key_fields:
                            consistency_context += f"  Record {i+1} ({entry.name}): {str(key_fields)[:200]}\n"
            except Exception:
                pass
            
            # Add context from related DocTypes (e.g., if generating Employee, show Company context)
            try:
                # Find Link fields that point to other DocTypes we've generated
                for field_info in meta.get_data_fields():
                    if field_info.fieldtype == "Link" and field_info.options:
                        related_doctype = field_info.options
                        related_records = self.record_cache.get_records(related_doctype)
                        if related_records:
                            sample_related = related_records[:2]
                            consistency_context += f"\nRelated {related_doctype} records available (for {field_info.fieldname} field): "
                            consistency_context += ", ".join([entry.name for entry in sample_related]) + "\n"
            except Exception:
                pass

            # BATCH PROMPT: Generate multiple records in one call
            prompt = (
                f"Generate {batch_size} realistic {locale_lang} mock data records for DocType: {doctype}.\n"
                + industry_context + "\n"
                + f"Locale: {locale}\n"
                + consistency_context + "\n"
                + "Fields:\n" + fields_text + "\n\n"
                + f"Rules: Return ONLY a valid JSON ARRAY with {batch_size} objects. No markdown, no backticks. "
                + f"All text in {locale_lang}. "
                + "Select fields: ONLY listed values. Link fields: ONLY listed names or null. "
                + "Check: 0 or 1. Required: " + req_text + ". "
                + "Make each record unique but consistent with the context above. "
                + "Ensure data is realistic for Turkish B2B marketplace.\n"
                + "Return format: [{{record1}}, {{record2}}, ...]\nJSON:"
            )

            # Initialize rate limiter
            from mock_data_engine.utils.rate_limiter import get_rate_limiter
            rate_limiter = get_rate_limiter(provider_name.lower())
            ...
            try:
                if provider_name.lower() == "gemini":
                    from mock_data_engine.providers.gemini_provider import GeminiProvider
                    from mock_data_engine.providers.ai_service import AIServiceConfig, AIProviderType
                    api_key = settings.get_password("gemini_api_key") if hasattr(settings, "gemini_api_key") else None
                    if not api_key:
                    ...
                    from mock_data_engine.providers.openai_provider import OpenAIProvider
                    from mock_data_engine.providers.ai_service import AIServiceConfig, AIProviderType
                    api_key = settings.get_password("openai_api_key")
                    model = getattr(settings, "openai_model", "gpt-4o-mini") or "gpt-4o-mini"
                    ai_config = AIServiceConfig(provider_type=AIProviderType.OPENAI, api_key=api_key, model=model, temperature=float(temperature))
                    provider = OpenAIProvider(ai_config)
                    provider.initialize()
                elif provider_name.lower() == "anthropic":
                    from mock_data_engine.providers.anthropic_provider import AnthropicProvider
                    from mock_data_engine.providers.ai_service import AIServiceConfig, AIProviderType
                    api_key = settings.get_password("anthropic_api_key")
                    model = getattr(settings, "anthropic_model", "claude-sonnet-4-20250514") or "claude-sonnet-4-20250514"
                    ai_config = AIServiceConfig(provider_type=AIProviderType.ANTHROPIC, api_key=api_key, model=model, temperature=float(temperature))
                    provider = AnthropicProvider(ai_config)
                    provider.initialize()
                else:
                    return None
                ...
                result = provider.generate_with_retry(
                    prompt=prompt,
                    context={"locale": locale, "doctype": doctype, "batch_size": batch_size}
                )
                
                # Record successful request (only 1 request for batch)
                ...
                if is_rate_limit:
                    wait_time = 60.0
                    frappe.log_error(
                        message=f"AI rate limit exceeded for {doctype} batch. Waiting {wait_time}s and falling back to Faker. Error: {str(e)[:200]}",
                        title="Mock AI Rate Limit - Fallback to Faker"
                    )
                    time.sleep(min(wait_time, 5.0))
                else:
                    frappe.log_error(
                        message=f"AI batch generation failed for {doctype}: {str(e)[:500]}",
                        title="Mock AI Batch Generation Error - Fallback to Faker"
                    )
            ...
            try:
                ...
                frappe.log_error(
                    message=f"Failed to parse AI batch response for {doctype}: {content[:200]}",
                    title="Mock AI Batch Parse Error"
                )
                return None

            # Validate and clean each record
            validated_records = []
            for data in data_list[:batch_size]:  # Limit to requested batch size
                # Validate Select fields
                for fn, opts in select_options.items():
                    if fn in data and data[fn] not in opts:
                        data[fn] = opts[0] if opts else None

                # Validate Link fields
                for fn, existing in link_options.items():
                    if fn in data and data[fn] not in existing:
                        data[fn] = existing[0] if existing else None

                # Validate type constraints
                for field_info in meta.get_data_fields():
                    ft = field_info.fieldtype
                    fn = field_info.fieldname
                    if ft == "Check" and fn in data:
                        data[fn] = 1 if data[fn] else 0
                    elif ft == "Int" and fn in data:
                        try:
                            data[fn] = int(data[fn]) if data[fn] is not None else 0
                        except (ValueError, TypeError):
                            data[fn] = 0
                    elif ft in ("Float", "Currency", "Percent") and fn in data:
                        try:
                            data[fn] = float(data[fn]) if data[fn] is not None else 0.0
                        except (ValueError, TypeError):
                            data[fn] = 0.0
            ...
            import frappe
            frappe.log_error(
                message=f"AI batch failed for {doctype}: {str(e)}",
                title="Mock AI Batch Fallback"
            )
            return None

    def _generate_with_ai(self, doctype, meta, config, record_idx):
        """Generate a complete record using AI with rate limiting and retry."""
        try:
            import frappe
            import json as json_mod
            import time
            ...
            # Read industry from profile with full context
            industry_context = ""
            industry_ai_context = None
            if hasattr(self, "_request_doc") and self._request_doc:
                profile_name = getattr(self._request_doc, "profile", None)
                if profile_name:
                    try:
                        ...
                        if getattr(profile, "industry", None):
                            try:
                                ind_doc = frappe.get_doc("Mock Industry", profile.industry)
                                # Get full AI context from industry
                                if hasattr(ind_doc, "get_ai_context"):
                                    industry_ai_context = ind_doc.get_ai_context()
                                
                                # Build detailed industry context string
                                industry_context = f"Industry: {ind_doc.industry_name or ind_doc.name}"
                                if getattr(ind_doc, "description", None):
                                    industry_context += f" - {ind_doc.description}"
                                
                                # Add detailed context hints if available
                                if industry_ai_context:
                                    if industry_ai_context.get("typical_company_types"):
                                        companies = ", ".join(industry_ai_context["typical_company_types"][:10])
                                        industry_context += f"\nTypical Company Types: {companies}"
                                    if industry_ai_context.get("typical_products_services"):
                                        products = ", ".join(industry_ai_context["typical_products_services"][:10])
                                        industry_context += f"\nTypical Products/Services: {products}"
                                    if industry_ai_context.get("typical_job_titles"):
                                        jobs = ", ".join(industry_ai_context["typical_job_titles"][:10])
                                        industry_context += f"\nTypical Job Titles: {jobs}"
                                    if industry_ai_context.get("typical_departments"):
                                        depts = ", ".join(industry_ai_context["typical_departments"][:10])
                                        industry_context += f"\nTypical Departments: {depts}"
                            except Exception as e:
                                frappe.log_error(
                                    message=f"Error loading industry context: {str(e)}",
                                    title="Mock Industry Context Error"
                                )
                                industry_context = "Industry: " + str(profile.industry)
                        prof_temp = getattr(profile, "ai_temperature", None)
                        if prof_temp and float(prof_temp) > 0:
                            temperature = float(prof_temp)
                    except Exception:
            ...
            # Get locale - check industry faker hints first, then profile/request, then default
            locale = self._locale or "tr_TR"
            if industry_ai_context and hasattr(self, "_request_doc") and self._request_doc:
                profile_name = getattr(self._request_doc, "profile", None)
                if profile_name:
                    try:
                        profile = frappe.get_doc("Mock Generation Profile", profile_name)
            ...
            prompt = (
                "Generate realistic " + locale_lang + " mock data for DocType: " + doctype + ".\n"
                + industry_context + "\n"
                + "Locale: " + locale + "\n"
                + "Record " + str(record_idx + 1) + " of " + str(config.count) + "\n"
                + consistency_context + "\n"
                + "Fields:\n" + fields_text + "\n\n"
                + "Rules: Return ONLY valid JSON object. No markdown, no backticks. All text in " + locale_lang + ". "
                + "Select fields: ONLY listed values. Link fields: ONLY listed names or null. "
                + "Check: 0 or 1. Required: " + req_text + ". "
                + "Make data realistic and consistent with the context above for Turkish B2B marketplace.\nJSON:"
            )
            ...
            # Create provider and use generate_with_retry
            provider = None
            try:
                if provider_name.lower() == "gemini":
                    from mock_data_engine.providers.gemini_provider import GeminiProvider
                ...
                # Use generate_with_retry which handles rate limits automatically
                result = provider.generate_with_retry(
                    prompt=prompt,
                    context={"locale": locale, "doctype": doctype}
                )
                ...
                # Record failed request (still counts toward rate limit)
                rate_limiter.record_request()
                
                # For rate limit errors, wait longer before next attempt
                if is_rate_limit:
                    # Wait for rate limit window (typically 60 seconds for free tier)
                    wait_time = 60.0
                    frappe.log_error(
                        message=f"AI rate limit exceeded for {doctype}. Waiting {wait_time}s and falling back to Faker. Error: {str(e)[:200]}",
                        title="Mock AI Rate Limit - Fallback to Faker"
                    )
                    time.sleep(min(wait_time, 5.0))  # Max 5 seconds wait in generation
                else:
                    # Log other errors
                    frappe.log_error(
                        message=f"AI generation failed for {doctype}: {str(e)[:500]}",
                        title="Mock AI Generation Error - Fallback to Faker"
                    )
                
                # Fall back to standard Faker generation
                return None
            ...
            import frappe
            frappe.log_error(
                message="AI failed for " + doctype + ": " + str(e),
                title="Mock AI Fallback"
            )
            return None
    ...
    def _generate_record_data(
        ...
        # AI generation is MANDATORY when enabled - no Faker fallback
        if config.use_ai and self._enable_ai_generation:
            ai_data = self._generate_with_ai(doctype, meta, config, record_idx)
            if ai_data:
                return ai_data
            else:
                # AI generation failed - raise error instead of falling back to Faker
                raise Exception(
                    f"AI generation failed for {doctype} record {record_idx + 1}. "
                    "AI generation is required - cannot fallback to Faker. "
                    "Please check API keys and rate limits."
                )

        # If AI is not enabled, use Faker (only when explicitly disabled)
    ...
    def _insert_record(
        ...
        try:
            ...
            # These flags bypass validation, permissions, and mandatory checks
            doc.flags.ignore_validate = True
            doc.flags.ignore_permissions = True
            doc.flags.ignore_mandatory = True
            doc.flags.ignore_links = True
            
            # Bypass custom validate() methods by setting a flag that some DocTypes check
            # This is a workaround for DocTypes with custom validation logic
            if hasattr(doc, 'flags'):
                doc.flags.skip_validate = True
                doc.flags.skip_before_save = True

            # Insert with all ignore flags
            # Note: insert() doesn't accept ignore_validate parameter, only flags work
            ...
            # For ValidationError, try to extract the actual message
            if "ValidationError" in error_type or "validate" in error_msg.lower():
                # Try to get more context from the error
                import traceback
                tb_str = traceback.format_exc()
                # Extract the actual validation message
                if "throw" in tb_str or "ValidationError" in tb_str:
                    # Look for the actual error message in traceback
                    lines = tb_str.split('\n')
                    for line in reversed(lines):
                        if "User is required" in line or "required" in line.lower():
                            error_msg = line.strip()
                            break
            
            # Re-raise with enhanced error message
            raise Exception(f"{error_type}: {error_msg}") from e
