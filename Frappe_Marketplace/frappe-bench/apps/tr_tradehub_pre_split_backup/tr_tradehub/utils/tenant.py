...
Usage in hooks.py:
    doc_events = {
        "*": {
            "before_insert": "tr_tradehub.utils.tenant.set_tenant",
            "validate": "tr_tradehub.utils.tenant.validate_tenant"
        }
    }

Usage in code:
    from tr_tradehub.utils.tenant import get_current_tenant, tenant_required
...
def get_permission_query_conditions(user: str = None) -> str:
    ...
    Usage in hooks.py:
        permission_query_conditions = {
            "SKU Product": "tr_tradehub.utils.tenant.get_permission_query_conditions"
        }
    """
    if not user:
        user = frappe.session.user
...
def has_permission(doc: "frappe.model.document.Document", user: str = None) -> bool:
    ...
    Usage in hooks.py:
        has_permission = {
            "SKU Product": "tr_tradehub.utils.tenant.has_permission"
        }
    """
    if not user:
        user = frappe.session.user
...
def get_my_tenant() -> Optional[dict]:
    ...
    Example API call:
        frappe.call('tr_tradehub.utils.tenant.get_my_tenant')
    """
    tenant = get_current_tenant()
    if not tenant:
        return None
